...
#    readout = tf.nn.softmax(tf.matmul(h_fc1, W_fc2) + b_fc2)  #(-1,2)
    readout = tf.matmul(h_fc1, W_fc2) + b_fc2
    g1,g2_temp=tf.split(1, 2, readout)
...
#                s_hit : s_j_batch})
        ...
        if t % 10000 == 0:
            saver.save(sess3, store_network_path + GAME + '-dqn', global_step = t+pretrain_number)
...
#            writer.add_summary(result, t+pretrain_number)
        ...
        print 'Total score:',total_score,' Positive_score:',positive_score,'   up:',readout_t[0],'    down:',readout_t[1],'  no:',readout_t[2],g1_num,g2_num
...
def load_weights():
    ...
    destination2="assigned_network/pong-dqn-2570000_train_cost"  
...
#    print new_variable_list[1]
    ...
    old_variable_normal=sess.run(old_variable)
...
#    print old_variable_normal[1],len(old_variable_normal)
    ...
    old_variable_train_hit_only=sess.run(old_variable)
    
#    print old_variable_train_hit_only[1],len(old_variable_train_hit_only)
    sess.close()
    
    return old_variable_normal,old_variable_train_hit_only
