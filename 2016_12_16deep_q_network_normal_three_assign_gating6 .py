...
def final_network():
    ...
    # readout layer
    readout = tf.matmul(h_fc1, W_fc2) + b_fc2  #(-1,2)
    
    g11,g12,g13,g21,g22,g23=tf.split(1, 6, readout)
    
    up_soft=tf.nn.softmax(tf.concat(1,[g11,g21]))
    down_soft=tf.nn.softmax(tf.concat(1,[g12,g22]))
    no_soft = tf.nn.softmax(tf.concat(1,[g13,g23]))
    
    g11,g21=tf.split(1,2,up_soft)
    g12,g22=tf.split(1,2,down_soft)
    g13,g23=tf.split(1,2,no_soft)
    ...
    out_up = tf.mul(g11,s1_up)+tf.mul(g21,s2_up)
    out_down =tf.mul(g12,s1_down)+tf.mul(g22,s2_down)
    out_no = tf.mul(g13,s1_no)+tf.mul(g23,s2_no)
    """"""""""""""""""""""""""""""""""""""""""""""""
