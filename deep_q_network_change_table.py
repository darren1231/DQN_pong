...
def trainNetwork(s,s_after, readout,readout_after, h_fc1, sess,sess2):
    ...
    #after_network
    a_after = tf.placeholder("float", [None, ACTIONS])
    y_after = tf.placeholder("float", [None])
    readout_action_after = tf.reduce_sum(tf.mul(readout_after, a_after), reduction_indices = 1)
    cost_after = tf.reduce_mean(tf.square(y_after - readout_action_after))
    train_step_after = tf.train.AdamOptimizer(1e-6).minimize(cost_after)
    ...
    while True:
        ...
        readout_t_after=readout_after.eval(session=sess2,feed_dict = {s_after : [s_t]})[0]
        ...
        if random.random() <= epsilon or t <= OBSERVE:
            ...
            if(hit_state==False):
                ...
                action_index = np.argmax(readout_t_after)
                a_t[action_index] = 1
        ...
        for i in range(0, K):
            ...
            if(hit_state==False):
                if r_t==0.5:
                    r_t=1
                    D_before_hit.append((s_t, a_t, r_t, s_t1, terminal))
                else:
                    D_before_hit.append((s_t, a_t, r_t, s_t1, terminal))
            ...
            if(hit_state==True):
                if r_t==0.5:
                    r_t=0
                    D_after_hit.append((s_t, a_t, r_t, s_t1, terminal))
                else:
                    D_after_hit.append((s_t, a_t, r_t, s_t1, terminal))
                
            if judge_r_t==-1 or judge_r_t==1:
                hit_state=False
            ...
            if len(D_before_hit) > REPLAY_MEMORY:
                D_before_hit.popleft()
            if len(D_after_hit) > REPLAY_MEMORY:
                D_after_hit.popleft()
        ...
        if t > OBSERVE:
            ...
            minibatch = random.sample(D_before_hit, BATCH)
            minibatch_after=random.sample(D_after_hit, BATCH)
            ...
            s_j_batch_after = [d[0] for d in minibatch_after]
            a_batch_after = [d[1] for d in minibatch_after]
            r_batch_after = [d[2] for d in minibatch_after]
            s_j1_batch_after = [d[3] for d in minibatch_after]
            ...
            readout_after_j1_batch = readout_after.eval(session=sess2,feed_dict = {s_after : s_j1_batch_after})
            ...
            for i in range(0, len(minibatch_after)):
                # if terminal only equals reward
                if minibatch_after[i][4]:
                    y_batch_after.append(r_batch_after[i])
                else:
                    #y_batch.append(r_batch[i] + GAMMA * np.max(readout_j1_batch[i]))
                    y_batch_after.append(r_batch_after[i] + GAMMA * np.max(readout_after_j1_batch[i]))             
            ...
            # perform gradient step
            sess2.run(train_step_after,feed_dict = {
                y_after : y_batch_after,
                a_after : a_batch_after,
                s_after : s_j_batch_after})
        ...
        if t % 500 == 0:  
            ...
            ','+str(total_score)+ ','+str(positive_score) +','+",".join([str(x) for x in readout_t_after]) \
            +','+time_text+'\n')
...
def playGame():
    ...
    s_after,readout_after,h_fc1_after = createNetwork()

#    merged = tf.merge_all_summaries()
#    writer = tf.train.SummaryWriter(tensorboard_path, sess.graph)
    
    trainNetwork(s,s_after, readout,readout_after, h_fc1, sess,sess2)
