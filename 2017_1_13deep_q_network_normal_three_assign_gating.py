...
def trainNetwork(s,s_hit, readout,readout_hit, sess,sess2,s1,s2,final_out,sess3,variable_normal,variable_hit,old_variable_normal,old_variable_train_hit_only,s_final,g1,g2):
    ...
    #define final cost function
    a_final = tf.placeholder("float", [None, ACTIONS])
    y_final = tf.placeholder("float", [None])
    readout_action_final = tf.reduce_sum(tf.mul(final_out, a_final), reduction_indices = 1)
    cost_final = tf.reduce_mean(tf.square(y_final - readout_action_final))
    train_step_final = tf.train.AdamOptimizer(1e-6).minimize(cost_final)
    ...
    ##load weights##############################################
    for i in range(len(variable_normal)):
        print variable_normal[i].get_shape(),
        
    print '\n'
    
    for i in range(len(variable_hit)):
        print variable_hit[i].get_shape(),

    print '\n'
    
    v=sess.run(variable_hit)
    print v[11],v[11].shape
    
    assing_op_normal=[tf.assign(variable_hit[i+10],old_variable_normal[i]) for i in range(10)]    
    assing_op_hit=[tf.assign(variable_hit[i+20],old_variable_train_hit_only[i]) for i in range(10)]
    
    sess.run(assing_op_normal)
    v=sess.run(variable_hit)
    print v[21],v[21].shape
    
    sess2.run(assing_op_hit)
    ###########################################################
    print "success loaded old weights"
    ...
    while True:
        ...
        g1_num  = sess3.run(g1,feed_dict = {s1 : [readout_t],s2 : [hit_t],s_final:[s_t]})[0][0]
        g2_num  = sess3.run(g2,feed_dict = {s1 : [readout_t],s2 : [hit_t],s_final:[s_t]})[0][0]
...
#                a_t[action_index_2] = 1
            action_index = np.argmax(final_t)
            a_t[action_index] = 1
...
#                    r_batch_temp[i]=0
                ...
                # if terminal only equals reward
                if minibatch[i][4]:
                    y_batch_final.append(r_batch_temp[i])
                else:
                    #y_batch.append(r_batch[i] + GAMMA * np.max(readout_j1_batch[i]))
                    y_batch_final.append(r_batch_temp[i] + GAMMA * np.max(readout_final_j1_batch[i])) 
...
#            writer.add_summary(result, t+pretrain_number)
            ...
            +','+time_text+','+",".join([str(x) for x in hit_t])+','+ \
            ",".join([str(x) for x in final_t])+','+str(g1_num)+','+str(g2_num)+'\n')
        ...
        print 'Total score:',total_score,' Positive_score:',positive_score,"G1:",g1_num,"  G2:",g2_num
...
#    destination1="assigned_network/pong-dqn-510000_normal"
#    destination2="assigned_network/pong-dqn-1320000_train_hit_only"  
    
    destination2="assigned_network/flappybird-dqn-790000_without_reward_alive"
    destination1="assigned_network/flappybird-dqn-1770000_without_pass_reward"  
    
    sess = tf.Session()
    
    s, readout,variable = createNetwork()
...
#    print new_variable_list[1]
    ...
    saver = tf.train.Saver()
    saver.restore(sess, destination1)
        
    old_variable = tf.all_variables()
    old_variable_reward_alive=sess.run(old_variable)
    
    
#    print old_variable_normal[1],len(old_variable_normal)
    
    
    saver.restore(sess, destination2)
    old_variable_pass_reward=sess.run(old_variable)
    
#    print old_variable_train_hit_only[1],len(old_variable_train_hit_only)
    sess.close()
    print "Success extract old weights"
    
    return old_variable_reward_alive,old_variable_pass_reward
...
def playGame():
    ...
    old_variable_normal,old_variable_train_hit_only=load_weights()
    
    s, readout,variable_normal = createNetwork()
    s_hit,readout_hit,variable_hit= createNetwork()
    
    
    
    
    s_final,s1,s2,final_out,g1,g2 = final_network()

#    merged = tf.merge_all_summaries()
#    writer = tf.train.SummaryWriter(tensorboard_path, sess.graph)
    
    
    trainNetwork(s,s_hit, readout,readout_hit, sess,sess2,s1,s2,final_out,sess3,variable_normal,variable_hit,old_variable_normal,old_variable_train_hit_only,s_final,g1,g2)
...
#    old_variable_normal,old_variable_train_hit_only=load_weights()
#    for i in range(len(old_variable_normal)):
#        print old_variable_normal[i].shape,
#    print '\n'
#    for i in range(len(old_variable_train_hit_only)):
#        print old_variable_train_hit_only[i].shape,
#   # load_weights(destination2)
