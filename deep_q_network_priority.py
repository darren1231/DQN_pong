...
def trainNetwork(s, readout, h_fc1, sess,merged,writer):
    ...
    while True:
        ...
        for i in range(0, K):
            ...
            if(r_t==1):
                D_priority.append((s_t, a_t, r_t, s_t1, terminal))
            else:                
            ...
            if len(D_priority) > REPLAY_MEMORY:
                D_priority.popleft()
        print len(D),len(D_priority)
        ...
        if t > OBSERVE:
            ...
            if len(D_priority)<16:
                priority_batch = random.sample(D_priority, len(D_priority))
                minibatch = random.sample(D, BATCH-len(D_priority))
                
                
            else:
                priority_batch = random.sample(D_priority, BATCH/2)
                minibatch = random.sample(D, BATCH/2)
                
            minibatch.extend(priority_batch)

            # get the batch variable
