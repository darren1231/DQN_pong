...
def trainNetwork(s, readout, h_fc1, sess,merged,writer):
    ...
    while True:
        ...
        for i in range(0, K):
            ...
            TD_error = abs(GAMMA*np.max(readout_t1)-p_t)
            
            # store the transition in D
            D.append((s_t, a_t, r_t, s_t1, terminal,TD_error))
        ...
        if t > OBSERVE:
            ...
            minibatch.sort(key=itemgetter(5),reverse=True)
#            readout_j_batch = readout.eval(feed_dict = {s : s_j_batch})
#            
            
#            TD_batch=[]
#            for i in range(0,len(minibatch)):
#                temp_a=np.array(readout_j_batch[i])
#                temp_b=np.array(a_batch[i])
#                
#                q_s_a=sum(temp_a*temp_b)
#                TD_batch.append(abs(r_batch[i] + GAMMA * np.max(readout_j1_batch[i])-q_s_a))
            
            ###sort
#            for i in range(0,len(minibatch)):
#                for j in range(0,len(minibatch)-1-i):
#                    if(TD_batch[j]<TD_batch[j+1]):
#                        temp=minibatch[j]
#                        minibatch[j+1]=minibatch[j]
#                        minibatch[j]=temp
#                        
#                        temp_TD=TD_batch[j]
#                        TD_batch[j+1]=TD_batch[j]
#                        TD_batch[j]=temp_TD
            ...
            final_a_batch = a_batch[0:BATCH]
            final_s_j_batch=s_j_batch[0:BATCH]
