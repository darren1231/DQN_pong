...
import pong_fun_modified as game# whichever is imported "as game" will be used
import tetris_fun
import random
import numpy as np
import os
...
def normal_distribution(mean,var):
    y=random.random()    
    ln_in = math.sqrt(2*var*math.pi)*y
    sqrt_in = -math.log(ln_in)*2*var
    x=math.sqrt(abs(sqrt_in))+mean
...
def trainNetwork(s, readout, h_fc1, sess,merged,writer):
    ...
    cost_sum = tf.reduce_sum(tf.square(y - readout_action))
    ...
    do_nothing[0] = 1
    x_t, r_0, terminal = game_state.frame_step(do_nothing,15)
    ...
    while True:
        ...
        for i in range(0, K):
            ...
            x_t1_col, r_t, terminal = game_state.frame_step(a_t,15+change_speed)
        ...
        # TD error
        readout_t1 = readout.eval(feed_dict = {s : [s_t1]})
        p_t = sum(readout_t*a_t)
        TD_error = GAMMA*np.max(readout_t1)-p_t
        
        hit = normal_distribution(p_t,abs(squre_error))     
        print 'Td_error:',hit-p_t
        change_speed = abs(hit-p_t)*10
        ...
        if t > OBSERVE:
            ...
            squre_error=cost_sum.eval(feed_dict = {
                y : y_batch,
                a : a_batch,
                s : s_j_batch})
                
            print 'squre_error:',squre_error,'change_speed:',change_speed
        ...
        if t % 500 == 0:  
            ...
            +','+time_text+','+str(TD_error)+','+str(change_speed)+','+str(squre_error)+'\n')
